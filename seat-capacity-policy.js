(function(root,factory){
 ...
 function currentParty(doc){if(normalizeStatus(doc)==='empty')return 0;const value=Number(doc?.partySize);return Number.isInteger(value)&&value>0?value:null}
 ...
 function evaluateZone({zone,partySize,tables}){
  ...
  const occupied=normalized.reduce((sum,table)=>{const value=currentParty(table.doc);return value===null?NaN:sum+value},0);
