...
function render(){const all=master.map(seatData),count=st=>all.filter(s=>s.status===st).length,availability=bottleAvailability(),renewal=availability.reason==='unsupported-year'||Date.now()>=Date.UTC(bottlePolicy.SUPPORTED_END_YEAR,10,2);document.getElementById('seatSummary').innerHTML=`${renewal?'<span class="policy-renewal">⚠ 공휴일 데이터 갱신 필요</span>':''}<span class="empty">🟢 사용가능 ${count('empty')}</span><span class="held">🟡 주문중 ${count('held')}</span><span class="occupied">🔴 사용중 ${count('occupied')}</span><span class="reserved">🟣 예약 ${count('reserved')}</span><button class="bulk-clean-button start" onclick="bulkAction()">전체 해제</button>`;document.getElementById('seatAdmin').innerHTML=zones.map(z=>`<section class="simple-zone seat-zone-${z.id}" data-seat-zone="${z.id}"><h2>${z.name}</h2><div class="simple-seat-grid">${all.filter(s=>s.zone===z.id).map(s=>{const pending=pendingSeatActions.has(s.id),blocked=!bottleActionAllowed(s.id),reservable=!blocked&&(s.status==='empty'||(s.status==='held'&&!s.orderId));return `<div class="simple-seat-shell"><button type="button" class="simple-seat ${s.status}${blocked?' outside-hours':''}" data-seat-id="${esc(s.id)}"${(pending||blocked&&s.status==='empty')?' disabled':''}><strong>${s.displayName}</strong><span>최대 ${s.capacity}인</span><em><i aria-hidden="true"></i>${statusNames[s.status]}</em>${blocked?`<small class="outside-hours-badge">${bottleAdminReason()}</small>`:''}</button>${reservable?`<button type="button" class="seat-reservation-action ${s.status==='held'?'warning':''}" data-reserve-seat="${esc(s.id)}" ${pending?'disabled':''}>${pending?'처리 중…':s.status==='held'?'⚠ 주문중 좌석 예약':'예약'}</button>`:''}${s.status==='reserved'?`<button type="button" class="seat-reservation-action cancel" data-cancel-reservation="${esc(s.id)}" ${pending?'disabled':''}>${pending?'처리 중…':'예약 취소'}</button>`:''}</div>`}).join('')}</div></section>`).join('');}
...
scheduleBottlePolicyRender();window.addEventListener?.('focus',render);document.addEventListener?.('visibilitychange',()=>{if(!document.hidden)render()});window.addEventListener?.('pagehide',()=>{if(typeof clearTimeout==='function')clearTimeout(bottlePolicyTimer)});
