...
const statusNames={empty:'빈자리',held:'주문중',occupied:'사용중',reserved:'예약'};
const statusIcons={empty:'🟢',held:'🟡',occupied:'🔴',reserved:'🟣'};
...
function normalizedSeatStatus(status){return ['held','occupied','reserved'].includes(status)?status:'empty'}
...
async function transitionReservation(id,from,to){
 ...
 try{
  ...
  await db.runTransaction(async transaction=>{
   ...
   if(normalizedSeatStatus(saved.status)!==from)throw new Error('SEAT_STATUS_CHANGED');
   transaction.set(ref,{status:to,zone:seat.zone,name:seat.name,capacity:seat.capacity,reservedAt:to==='reserved'?firebase.firestore.FieldValue.serverTimestamp():null,reservedBy:to==='reserved'?'admin':null,updatedAt:firebase.firestore.FieldValue.serverTimestamp()},{merge:true});
  });
...
function reserveSeat(id){return transitionReservation(id,'empty','reserved')}
...
function render(){const all=master.map(seatData),count=st=>all.filter(s=>s.status===st).length;document.getElementById('seatSummary').innerHTML=`<span class="empty">🟢 사용가능 ${count('empty')}</span><span class="held">🟡 주문중 ${count('held')}</span><span class="occupied">🔴 사용중 ${count('occupied')}</span><span class="reserved">🟣 예약 ${count('reserved')}</span><button class="bulk-clean-button start" onclick="bulkAction()">전체 해제</button>`;document.getElementById('seatAdmin').innerHTML=zones.map(z=>`<section class="simple-zone seat-zone-${z.id}" data-seat-zone="${z.id}"><h2>${z.name}</h2><div class="simple-seat-grid">${all.filter(s=>s.zone===z.id).map(s=>{const pending=pendingSeatActions.has(s.id);return `<div class="simple-seat-shell"><button type="button" class="simple-seat ${s.status}" data-seat-id="${esc(s.id)}" ${pending?'disabled':''}><strong>${s.displayName}</strong><span>최대 ${s.capacity}인</span><em><i aria-hidden="true"></i>${statusNames[s.status]}</em></button>${s.status==='empty'?`<button type="button" class="seat-reservation-action" data-reserve-seat="${esc(s.id)}" ${pending?'disabled':''}>${pending?'처리 중…':'예약'}</button>`:''}${s.status==='reserved'?`<button type="button" class="seat-reservation-action cancel" data-cancel-reservation="${esc(s.id)}" ${pending?'disabled':''}>${pending?'처리 중…':'예약 취소'}</button>`:''}</div>`}).join('')}</div></section>`).join('');}
