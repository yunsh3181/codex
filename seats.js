...
const {ADMIN_SEAT_STATUSES,normalizeAdminSeatStatus,getAdminSeatActions,transitionAdminSeatState}=PJAdminOperations;
...
function render(){
 ...
 document.getElementById('seatAdmin').innerHTML=slotEntries(positions).map(({slot,seatId})=>{
  ...
  const actionButtons=actions.length?`<div class="admin-seat-actions" aria-busy="${pending}">${actions.map(action=>`<button type="button" class="admin-seat-action ${action.className}" data-seat-transition="${action.target}" data-seat-from="${s.status}" data-seat-id="${esc(s.id)}" ${pending||blocked&&action.target!=='empty'?'disabled':''} aria-label="${esc(s.displayName)} ${statusNames[s.status]}. ${action.label}">${action.label}</button>`).join('')}</div>`:s.status==='held'&&s.orderId?`<div class="admin-seat-actions"><button type="button" class="admin-seat-action empty" data-held-order="${esc(s.id)}" aria-label="${esc(s.displayName)} 주문 상세">주문 상세</button></div>`:'';
...
seatAdmin?.addEventListener('click',event=>{if(layoutEditing)return;const action=event.target.closest?.('[data-seat-transition],[data-held-order]');if(!action||action.disabled||!seatAdmin.contains(action))return;event.stopPropagation();if(action.dataset.heldOrder)openHeldSeatOrder(action.dataset.heldOrder);else runSeatAction(action.dataset.seatId,action.dataset.seatFrom,action.dataset.seatTransition)});
