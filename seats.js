...
let savedPositions=copyDefault(),draftPositions=copyDefault(),layoutRevision=0,layoutEditing=false,layoutSaving=false,layoutWarning='',draggedSeatId=null,dragTargetSlot=null,unsubscribeSeatLayout=null;
...
function layoutDirty(){return layoutEditing&&!positionsEqual(savedPositions,draftPositions)}
...
function renderLayoutControls(){
 const edit=document.getElementById('editSeatLayout'),save=document.getElementById('saveSeatLayout'),cancel=document.getElementById('cancelSeatLayout'),reset=document.getElementById('resetSeatLayout');
 if(!edit)return;edit.hidden=layoutEditing;save.hidden=cancel.hidden=reset.hidden=!layoutEditing;save.disabled=layoutSaving||!layoutDirty();cancel.disabled=reset.disabled=layoutSaving;
...
function startLayoutEdit(){if(layoutSaving)return;closeOccupancyDialog();closeOccupiedSeatDialog();layoutEditing=true;draftPositions={...savedPositions};render();document.querySelector('[data-seat-move-handle]')?.focus()}
function cancelLayoutEdit(){if(layoutSaving)return;draftPositions={...savedPositions};layoutEditing=false;draggedSeatId=null;render();document.getElementById('editSeatLayout')?.focus()}
...
async function saveLayout(){
 ...
 try{
  const expectedRevision=layoutRevision,nextPositions={...checked.positions},uid=firebase.auth().currentUser?.uid;
  ...
  savedPositions=nextPositions;draftPositions={...nextPositions};layoutRevision=expectedRevision+1;layoutEditing=false;layoutWarning='';render();document.getElementById('editSeatLayout')?.focus();return true;
 }catch(error){
  if(error?.message==='SEAT_LAYOUT_STALE'){layoutWarning='다른 관리자가 먼저 저장했습니다. 최신 배열을 다시 불러온 뒤 수정해 주세요.';layoutEditing=false;draftPositions={...savedPositions}}
...
function applyLayoutSnapshot(snapshot){
 if(!snapshot.exists){savedPositions=copyDefault();layoutRevision=0;layoutWarning='';if(!layoutEditing)draftPositions={...savedPositions};render();return}
 const data=snapshot.data(),checked=validatePositions(data.positions);
 if(!checked.valid||!Number.isInteger(data.revision)||data.revision<1){savedPositions=copyDefault();layoutRevision=0;layoutWarning='저장된 좌석 배열이 손상되어 기본 배열로 표시합니다. 확인이 필요합니다.';if(!layoutEditing)draftPositions={...savedPositions};render();return}
 savedPositions=checked.positions;layoutRevision=data.revision;layoutWarning='';if(!layoutEditing)draftPositions={...savedPositions};render();
}
...
window.manageSeat=manageSeat;window.touchSeat=manageSeat;window.bulkAction=bulkAction;window.PJSeatLayoutEditor={startLayoutEdit,cancelLayoutEdit,resetLayoutDraft,applyDraftMove,saveLayout,applyLayoutSnapshot,getState:()=>({savedPositions:{...savedPositions},draftPositions:{...draftPositions},revision:layoutRevision,editing:layoutEditing,saving:layoutSaving,dirty:layoutDirty(),warning:layoutWarning})};
...
document.getElementById('editSeatLayout')?.addEventListener('click',startLayoutEdit);document.getElementById('saveSeatLayout')?.addEventListener('click',saveLayout);document.getElementById('cancelSeatLayout')?.addEventListener('click',cancelLayoutEdit);document.getElementById('resetSeatLayout')?.addEventListener('click',resetLayoutDraft);
