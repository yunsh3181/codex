...
test('actual Chromium unlocks and reuses one completion AudioContext without duplicate alerts',{timeout:120000},t=>{
 const report=path.join(os.tmpdir(),`waiting-tv-completion-${process.pid}.json`),screenshot=path.join(os.tmpdir(),`waiting-tv-completion-${process.pid}.png`),mixedScreenshot=path.join(os.tmpdir(),`waiting-tv-completion-mixed-${process.pid}.png`);t.after(()=>{fs.rmSync(report,{force:true});fs.rmSync(screenshot,{force:true});fs.rmSync(mixedScreenshot,{force:true})});
 const run=spawnElectronSync(require('electron'),['scripts/verify-waiting-tv-completion.js'],{cwd:root,encoding:'utf8',env:{...process.env,WAITING_TV_REPORT:report,WAITING_TV_SCREENSHOT:screenshot,WAITING_TV_MIXED_SCREENSHOT:mixedScreenshot,ELECTRON_DISABLE_SECURITY_WARNINGS:'true'},timeout:110000,maxBuffer:10*1024*1024});assert.equal(run.status,0,electronResultDetails(run));assert.equal(run.signal,null,electronResultDetails(run));
