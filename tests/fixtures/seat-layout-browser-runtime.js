(()=>{
 ...
 const saved=()=>{try{return JSON.parse(localStorage.getItem('seatLayoutFixture')||'null')}catch{return null}};
 const writeCount=()=>Number(localStorage.getItem('seatLayoutFixtureWriteCount')||0);
 ...
 window.__seatLayoutFixture={db,seatData,get writeCount(){return writeCount()},server(){return saved()},setServer(data){localStorage.setItem('seatLayoutFixture',JSON.stringify(data));notify()},clear(){localStorage.removeItem('seatLayoutFixture');localStorage.removeItem('seatLayoutFixtureWriteCount')}};
})();
