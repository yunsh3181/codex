...
test('iPad Air 3 portrait review uses one complete scrollable card column', {timeout:240000}, t=>{
 ...
 assert.equal(report.dockMatrix.length,6*3);for(const row of report.dockMatrix){const m=row.metrics,ctx=`${row.locale}/${row.scenario}`;assert.ok(m.dock.top-m.stage.bottom>=16,`${ctx} dynamic dock gap`);assert.equal(parseInt(row.reservedHeight,10),Math.ceil(m.dock.height),`${ctx} measured dock height`);assert.ok(m.confirmCenterError<=1,`${ctx} action center`)}
