...
assert.strictEqual(cssVersion,'49.0.3','changed administrator CSS bypasses the pre-audit browser cache');
assert.strictEqual(jsVersion,'49.1.3','administrator JavaScript cache version fetches independent extra quantities');
...
assert.equal((html.match(/admin\.css\?v=49\.0\.3/g)||[]).length,1,'admin dashboard loads the inquiry-alert CSS cache key exactly once');
