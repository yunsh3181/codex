...
if(!emulatorAvailable){test('admin takeout production transactions and rules matrix',{skip:true},()=>{});}else{
 ...
 function compatDb(db,mutations=[],transform=(path,data)=>data){const wrap=documentRef=>({native:documentRef,path:documentRef.path,collection:name=>({doc:id=>wrap(doc(db,documentRef.path,name,id))})});return {collection:name=>({doc:id=>wrap(ref(db,name,id))}),runTransaction:callback=>runTransaction(db,transaction=>callback({get:documentRef=>transaction.get(documentRef.native).then(snapshot=>({exists:snapshot.exists(),data:()=>snapshot.data()})),set(documentRef,data,options){mutations.push({type:'set',path:documentRef.path});const next=transform(documentRef.path,data);if(next!==null)transaction.set(documentRef.native,next,options)},update(documentRef,data){mutations.push({type:'update',path:documentRef.path});const next=transform(documentRef.path,data);if(next!==null)transaction.update(documentRef.native,next)},delete(documentRef){mutations.push({type:'delete',path:documentRef.path});transaction.delete(documentRef.native)}}))};}
 ...
 const auditContext=source=>({transitionSource:source,transactionStartedAt:Timestamp.fromMillis(1),clientInstanceId:'emulator-client',appVersion:'admin-test',actorRole:'admin'});
 const complete=(db,value,mutations=[])=>operations.completeTakeoutTransaction({db:compatDb(db,mutations),orderId:value.id,expectedStatus:value.status,serverTimestamp,adminId:'admin-one',...auditContext('admin_manual_ready')});
 const pickup=(db,value,mutations=[],adminId='admin-one')=>operations.completeTakeoutPickupTransaction({db:compatDb(db,mutations),orderId:value.id,expectedStatus:value.status,serverTimestamp,adminId,...auditContext('admin_manual_pickup')});
 ...
 const startPreparationChanged=(db,id,minutes,now,change,mutations=[])=>operations.startTakeoutPreparationTransaction({db:compatDb(db,mutations,(documentPath,data)=>change(documentPath,{...data})),orderId:id,preparationMinutes:minutes,nowMillis:now,serverTimestamp,timestampFromMillis:Timestamp.fromMillis,adminId:'admin-one',resolveBusinessDay:()=> '2026-08-08'});
 const autoComplete=(db,order,now,mutations=[])=>operations.autoCompleteTakeoutTransaction({db:compatDb(db,mutations),orderId:order.id,expectedReadyDueAt:order.readyDueAt,expectedPreparationStartedAt:order.preparationStartedAt,nowMillis:now,serverTimestamp,adminId:'admin-one',resolveBusinessDay:()=> '2026-08-08',...auditContext('takeout_auto_ready')});
 ...
 test('prep A-B. payment_pending and new takeout orders start cooking with an exact deadline',async()=>{
  ...
  for(const [index,status] of ['payment_pending','new'].entries()){
   ...
   assert.equal(result.orderWrites,1);assert.equal(result.displayWrites,1);assert.equal(result.seatWrites,0);assert.equal(result.paymentCalls,0);assert.equal(order.status,'cooking');assert.equal(order.preparationMinutes,15);assert.equal(order.preparationStartedAt.toMillis(),now);assert.equal(order.readyDueAt.toMillis(),now+15*60000);assert.equal(order.autoReadyEnabled,true);assert.equal(display.preparationMinutes,15);assert.equal(display.preparationStartedAt.toMillis(),order.preparationStartedAt.toMillis());assert.equal(display.readyDueAt.toMillis(),order.readyDueAt.toMillis());assert.equal(display.autoReadyEnabled,true);assert.equal((await getDocs(collection(adminDb(),'seats'))).size,0);assert.equal(index>=0,true);
  }
 });
 ...
 test('prep E. every allowed five-minute boundary uses one atomic order/display pair',async()=>{
  for(const minutes of [5,10,15,20,25,60]){const id=`prep-minutes-${minutes}`;await seedTakeout(takeout(id,'payment_pending'));const mutations=[],now=1770000000000+minutes;const result=await startPreparation(adminDb(),id,minutes,now,mutations),order=await read(adminDb(),'orders',id),display=await read(adminDb(),'publicOrderDisplays',id);assert.equal(result.orderWrites,1);assert.equal(result.displayWrites,1);assert.deepEqual(mutations.map(item=>item.path).sort(),[`orders/${id}`,`publicOrderDisplays/${id}`].sort());assert.equal(order.preparationMinutes,minutes);assert.equal(order.preparationStartedAt.toMillis(),now);assert.equal(order.readyDueAt.toMillis()-order.preparationStartedAt.toMillis(),minutes*60000);assert.equal(display.readyDueAt.toMillis(),order.readyDueAt.toMillis())}
 });
 test('prep F-H. invalid type, range, and deadline payloads are denied with zero committed writes',async()=>{
  const cases=[
   ['zero','order',{preparationMinutes:0}],['four','order',{preparationMinutes:4}],['seven','order',{preparationMinutes:7}],['sixty-one','order',{preparationMinutes:61}],['string-minutes','order',{preparationMinutes:'15'}],['null-minutes','order',{preparationMinutes:null}],
   ['started-string','both',{preparationStartedAt:'invalid'}],['due-string','both',{readyDueAt:'invalid'}],['deadline-mismatch','both',{readyDueAt:Timestamp.fromMillis(1770000000000+14*60000)}],['auto-false','both',{autoReadyEnabled:false}]
  ];
  for(const [name,target,override] of cases){const id=`prep-invalid-${name}`;await seedTakeout(takeout(id,'payment_pending'));await assert.rejects(startPreparationChanged(adminDb(),id,15,1770000000000,(documentPath,data)=>target==='both'||(target==='order'&&documentPath.startsWith('orders/'))?{...data,...override}:data));const order=await read(adminDb(),'orders',id),display=await read(adminDb(),'publicOrderDisplays',id);assert.equal(order.status,'payment_pending');assert.equal(order.preparationMinutes,undefined);assert.equal(display.preparationMinutes,undefined)}
 });
 test('prep I-J. omitted or mismatched public display aborts the order half',async()=>{
  for(const mode of ['omitted','missing','number-mismatch']){const id=`prep-display-${mode}`;await seedTakeout(takeout(id,'payment_pending'));if(mode==='missing')await environment.withSecurityRulesDisabled(context=>deleteDoc(ref(context.firestore(),'publicOrderDisplays',id)));await assert.rejects(startPreparationChanged(adminDb(),id,15,1770000000000,(documentPath,data)=>documentPath.startsWith('publicOrderDisplays/')?(mode==='number-mismatch'?{...data,orderNumber:'P9999'}:null):data));const order=await read(adminDb(),'orders',id),display=await read(adminDb(),'publicOrderDisplays',id);assert.equal(order.status,'payment_pending');assert.equal(order.preparationMinutes,undefined);if(mode==='missing')assert.equal(display,null);else assert.equal(display.preparationMinutes,undefined)}
 });
 test('prep K-N. stale states and immutable commercial fields cannot be changed',async()=>{
  for(const status of ['completed','ready','cancelled']){const id=`prep-stale-start-${status}`;await seedTakeout(takeout(id,status));await assert.rejects(startPreparation(adminDb(),id,15,1770000000000),{code:'order/stale-state'});assert.equal((await read(adminDb(),'orders',id)).status,status)}
  const changes=[['price',{total:1}],['discount',{discountAmount:1}],['items',{items:[]}],['payment',{payment:{method:'cash'}}],['orderNo',{orderNo:'P9999'}],['customerNumber',{customerNumber:'P9999'}],['source',{source:'other'}],['channel',{channel:'admin'}]];
  for(const [name,override] of changes){const id=`prep-protected-${name}`;await seedTakeout(takeout(id,'payment_pending'));await assert.rejects(startPreparationChanged(adminDb(),id,15,1770000000000,(documentPath,data)=>documentPath.startsWith('orders/')?{...data,...override}:data));const order=await read(adminDb(),'orders',id);assert.equal(order.status,'payment_pending');assert.equal(order.preparationMinutes,undefined)}
 });
 test('prep non-admin. ordinary user cannot start preparation and commits zero writes',async()=>{
  const id='prep-non-admin';await seedTakeout(takeout(id,'payment_pending'));await assert.rejects(startPreparation(userDb(),id,15,1770000000000));const order=await read(adminDb(),'orders',id),display=await read(adminDb(),'publicOrderDisplays',id);assert.equal(order.status,'payment_pending');assert.equal(order.preparationMinutes,undefined);assert.equal(display.preparationMinutes,undefined);
 });
 ...
 test('auto G-I. deadline before, at, and after follows the production transaction boundary',async()=>{
  ...
  for(const [suffix,now,success] of [['before',due-1,false],['at',due,true],['after',due+1,true]]){
   const id=`auto-${suffix}`;await seedTakeout(takeout(id,'payment_pending'));await startPreparation(adminDb(),id,15,due-900000);const order=await read(adminDb(),'orders',id),mutations=[];
 ...
 test('auto J-N. races, stale states/timers, catch-up, and repeated snapshots complete once',async()=>{
  const due=1770000900000,start=Timestamp.fromMillis(due-900000),id='auto-race';await seedTakeout(takeout(id,'payment_pending'));await startPreparation(adminDb(),id,15,due-900000);const order={id,...await read(adminDb(),'orders',id)};
  ...
  for(const status of ['ready','completed','cancelled']){const staleId=`auto-stale-${status}`;await seedTakeout(takeout(staleId,status));const mutations=[];await assert.rejects(operations.autoCompleteTakeoutTransaction({db:compatDb(adminDb(),mutations),orderId:staleId,expectedReadyDueAt:Timestamp.fromMillis(due),nowMillis:due,serverTimestamp,adminId:'admin-one',...auditContext('takeout_auto_ready')}),{code:'order/stale-state'});assert.equal(mutations.length,0)}
  const changedId='auto-changed-deadline';await seedTakeout(takeout(changedId,'payment_pending'));await startPreparation(adminDb(),changedId,15,due-840000);const changedMutations=[];await assert.rejects(operations.autoCompleteTakeoutTransaction({db:compatDb(adminDb(),changedMutations),orderId:changedId,expectedReadyDueAt:Timestamp.fromMillis(due),expectedPreparationStartedAt:start,nowMillis:due+60000,serverTimestamp,adminId:'admin-one',...auditContext('takeout_auto_ready')}),{code:'order/stale-timer'});assert.equal(changedMutations.length,0);
  const catchupId='auto-catchup';await seedTakeout(takeout(catchupId,'payment_pending'));await startPreparation(adminDb(),catchupId,15,due-900000);const catchup={id:catchupId,...await read(adminDb(),'orders',catchupId)},repeated=await Promise.allSettled([autoComplete(adminDb(),catchup,due+5000),autoComplete(adminDb('admin-two'),catchup,due+5000),autoComplete(adminDb(),catchup,due+5000)]);assert.equal(repeated.filter(result=>result.status==='fulfilled').length,1);assert.equal((await read(adminDb(),'orders',catchupId)).status,'ready');assert.equal((await getDocs(collection(adminDb(),'publicOrderDisplays'))).docs.filter(doc=>doc.id===catchupId).length,1);
 });
 test('auto recovery A-B. transient failure retries once through the production completion helper',async()=>{
  const due=1770000900000,start=Timestamp.fromMillis(due-900000),id='auto-transient-recovery';await seedTakeout(takeout(id,'payment_pending'));await startPreparation(adminDb(),id,15,due-900000);let current={id,...await read(adminDb(),'orders',id)},attempts=0,nextTimer=1,permanentError=null;const timers=new Map(),mutations=[];
  const coordinator=operations.createAutoReadyCoordinator({getCurrentOrder:()=>current,now:()=>due+1,setTimer:(callback,delay)=>{const timer=nextTimer++;timers.set(timer,{callback,delay});return timer},clearTimer:timer=>timers.delete(timer),onPermanentError:error=>{permanentError=error},execute:async (order,{retry=false}={})=>{attempts++;if(attempts===1)throw Object.assign(new Error('temporary outage'),{code:'unavailable'});const result=await operations.autoCompleteTakeoutTransaction({db:compatDb(adminDb(),mutations),orderId:order.id,expectedReadyDueAt:order.readyDueAt,expectedPreparationStartedAt:order.preparationStartedAt,nowMillis:due+1,serverTimestamp,adminId:'admin-one',resolveBusinessDay:()=> '2026-08-08',...auditContext(retry?'takeout_auto_retry':'takeout_auto_ready')});current={...current,status:'ready',autoReadyEnabled:false};return result}});
  ...
  if(permanentError)throw permanentError;const saved=await read(adminDb(),'orders',id),display=await read(adminDb(),'publicOrderDisplays',id);assert.equal(attempts,2);assert.equal(saved.status,'ready');assert.equal(saved.lastTransitionSource,'takeout_auto_retry');assert.equal(display.displayStatus,'ready');assert.deepEqual(mutations.map(item=>item.path).sort(),[`orders/${id}`,`orders/${id}/lifecycleAudit/takeout_${id}__cooking__ready`,`publicOrderDisplays/${id}`].sort());assert.equal((await getDocs(collection(adminDb(),'seats'))).size,0);
 });
}
