...
test('production kiosk passes the supported iPad Air 3 portrait integration', {timeout:180000}, t=>{
 ...
 assert.equal(report.safariResults.length,2*7);
 ...
 assert.equal(report.scrollResults.length,2*8);
 ...
 assert.deepEqual(report.dynamicResults.map(x=>x.metrics.visualViewport.height),[1112,940]);
 ...
 assert.equal(report.homeResults.length,3*6);
 for(const row of report.homeResults){const m=row.metrics,ctx=`Home/${row.viewport}/${row.locale}`;assert.equal(m.count,2,ctx);assert.ok([1,2].includes(m.columns),ctx);assert.ok(m.happyTakeoutOnly,ctx);assert.equal(m.heineken,false,ctx);assert.equal(m.horizontalOverflow,0,ctx);assert.deepEqual(m.clipped,[],ctx)}
