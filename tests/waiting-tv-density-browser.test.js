...
test('actual waiting-TV Chromium applies independent adaptive density without clipping or document overflow',{timeout:180000},t=>{
 ...
 for(const [viewport,samples] of Object.entries(result.viewports))for(const [name,sample] of Object.entries(samples)){
  ...
  assert.equal(sample.cooking.clipping,false,`${viewport} ${name} cooking clipping: ${JSON.stringify(sample.cooking.elements)}`);assert.equal(sample.ready.clipping,false,`${viewport} ${name} ready clipping: ${JSON.stringify(sample.ready.elements)}`);assert.equal(sample.cooking.overlap,false);assert.equal(sample.ready.overlap,false);assert.equal(sample.cooking.verticalKorean,false);assert.equal(sample.ready.verticalKorean,false);
