...
test('actual admin DOM preserves click, double-click, keyboard, focus, pagination, listener, and deletion behavior',{timeout:120000},t=>{
 ...
 assert.equal(result.reservationModal.title,'예약 주문 결제 유형');assert.deepEqual(result.reservationModal.choices,['결제완료 예약','후결제 예약']);assert.equal(result.reservationModal.focus,'prepaid');assert.equal(result.reservationModal.clipped,false);assert.equal(result.reservationModal.overflowX,0);assert.equal(result.reservationModal.transactions,0);assert.deepEqual(result.reservationModal.cancel,{hidden:true,focusAction:'select-reservation-payment'});assert.equal(result.reservationModal.success.status,'reservation_pending');assert.equal(result.reservationModal.success.paymentType,'pay_on_pickup');assert.equal(result.reservationModal.success.enabled,true);assert.match(result.reservationModal.success.label,/예약중.*후결제 예약/);assert.equal(result.reservationModal.success.modalHidden,true);assert.equal(result.reservationModal.success.transactions,1);assert.equal(result.reservationModal.success.display,false);
