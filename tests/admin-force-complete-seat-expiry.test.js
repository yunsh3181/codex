...
const operations=fs.readFileSync(path.join(root,'admin-operations.js'),'utf8');
...
test('force completion is centrally classified and writes only the order document',()=>{
 assert.match(admin,/forceCompleteTransaction\(\{db,orderId:id/);
 for(const status of ['payment_pending','new','accepted','paid','cooking','ready'])assert.ok(operations.includes(`'${status}'`));
 assert.match(operations,/if\(allHeld\|\|allOccupied\)return \{forceEligible:false/);
 assert.match(operations,/belongs\(record\)&&!\['held','occupied'\]\.includes\(record\.status\)/);
 const forceSource=admin.match(/async function forceCompleteOrder\(\)\{[\s\S]*?\n\}/)?.[0]||'';
 assert.match(operations,/transaction\.update\(orderRef,\{status:'completed'/);
 ...
 assert.match(operations,/adminForceCompleted:true/);
});
...
test('force confirmation modal is accessible, guarded, and action clicks stay isolated',()=>{
 ...
 assert.match(operations,/value\.slice\(-Math\.min\(4,value\.length\)\)/);
...
test('occupied expiry uses the exact three-hour boundary and grouped seat-only transactions',()=>{
 assert.match(operations,/OCCUPIED_EXPIRY_MS=3\*60\*60\*1000/);
 assert.match(operations,/millis<=now-OCCUPIED_EXPIRY_MS/);
 assert.match(operations,/group\.every\(seat=>\{const millis=timestampMillis\(seat\.occupiedAt\);return seat\.status==='occupied'/);
 assert.match(operations,/current\.status!=='occupied'/);
 assert.match(operations,/millis!==timestampMillis\(initial\.occupiedAt\)/);
 assert.match(operations,/transaction\.set\(ref,seatReleasePayload\(serverTimestamp\)/);
 const releaseSource=operations.match(/async function releaseExpiredSeatGroupTransaction[\s\S]*?\n \}/)?.[0]||'';
