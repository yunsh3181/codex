...
test('drink volume, included sauce and independent quantities work in the real renderer',{timeout:120000},t=>{
 ...
 assert.equal(result.drinkRows.rows.length,8);assert.equal(result.drinkRows.clipped,0,JSON.stringify(result.drinkRows.clippedDetails));assert.deepEqual(result.drinkRows.documentOverflow,[0,0]);for(const row of result.drinkRows.rows){assert.match(row.volume,/^(?:500mL|1\.(?:25|5)L)$/);assert.match(row.price,/^(?:1,800|2,500)원$/);assert.equal(row.volumeColor,'rgb(255, 255, 255)');assert.equal(row.quantityColor,'rgb(17, 17, 17)');assert.ok(row.volumeRect.right<=row.minusRect.left);assert.ok(row.minusRect.width>=44&&row.minusRect.height>=44);assert.ok(row.plusRect.width>=44&&row.plusRect.height>=44)}
 ...
 assert.equal(result.finalSideFlow.paths.every(row=>row.modal==='includedSauce'),true);assert.equal(result.finalSideFlow.modal,'includedSauce');assert.equal(result.finalSideFlow.count,1);assert.deepEqual(result.finalSideFlow.labels,['추가안함','추가선택']);assert.equal(result.finalSideFlow.extrasStep,'accompaniment');assert.equal(result.finalSideFlow.existingCartModal,'includedSauce');assert.deepEqual(result.finalSideFlow.noExtra,{step:'review',modal:null,orders:2});
 ...
 assert.deepEqual(result.firestoreIsolation,{externalRequests:0,requests:[],operatingReads:0,operatingWrites:0,authAttempts:0,listenerMode:'test-stub'});
 assert.equal(result.viewportMatrix.length,3*6);for(const row of result.viewportMatrix){const name=`${row.width}x${row.height}/${row.locale}`;assert.equal(row.review.documentOverflow[0],0,name);assert.deepEqual(row.review.clipped,[],name);assert.equal(row.review.lastReachable,true,name);assert.equal(row.review.paidZero,0,name);assert.ok(row.review.extraTotal,name);assert.equal(row.drink.documentOverflow[0],0,name);assert.deepEqual(row.drink.clipped,[],name);assert.equal(row.drink.buttons,true,name);assert.equal(row.modal.documentOverflow[0],0,name);assert.equal(row.modal.clipped,0,name);assert.equal(row.modal.highlight,true,name);assert.equal(row.modal.buttons,true,name);assert.ok(row.modal.rect[0]>=0&&row.modal.rect[1]>=0&&row.modal.rect[2]<=row.width&&row.modal.rect[3]<=row.height,name)}
