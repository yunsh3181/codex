...
const measure=`(()=>{
 ...
 const details=e=>{if(!e||!visible(e))return null;const node=[...e.childNodes].find(n=>n.nodeType===Node.TEXT_NODE&&n.data.trim()),lines=new Map;if(node)for(let i=0;i<node.data.length;i++){const range=document.createRange();range.setStart(node,i);range.setEnd(node,i+1);const r=range.getBoundingClientRect(),key=Math.round(r.top),line=lines.get(key)||{left:r.left,right:r.right};line.left=Math.min(line.left,r.left);line.right=Math.max(line.right,r.right);lines.set(key,line)}const textLineMaxWidth=Math.max(0,...[...lines.values()].map(line=>line.right-line.left));return {text:e.textContent.trim(),rect:rect(e),fontSize:+parseFloat(getComputedStyle(e).fontSize).toFixed(2),lineHeight:getComputedStyle(e).lineHeight,client:[e.clientWidth,e.clientHeight],scroll:[e.scrollWidth,e.scrollHeight],textLineMaxWidth:+textLineMaxWidth.toFixed(2),horizontalTextSafety:+(e.clientWidth-textLineMaxWidth).toFixed(2)}};
 const cards=[...document.querySelectorAll('.heroPromoStrip>.heroPromo')];
 const cardMetrics=cards.map(card=>{const children=[...card.children].filter(visible),r=card.getBoundingClientRect(),left=Math.min(...children.map(e=>e.getBoundingClientRect().left)),right=Math.max(...children.map(e=>e.getBoundingClientRect().right)),top=Math.min(...children.map(e=>e.getBoundingClientRect().top)),bottom=Math.max(...children.map(e=>e.getBoundingClientRect().bottom)),overlaps=[];for(let i=0;i<children.length;i++)for(let j=i+1;j<children.length;j++){const a=children[i].getBoundingClientRect(),b=children[j].getBoundingClientRect();if(a.left<b.right-.5&&a.right>b.left+.5&&a.top<b.bottom-.5&&a.bottom>b.top+.5)overlaps.push([children[i].className||children[i].tagName,children[j].className||children[j].tagName])}return {className:card.className,rect:rect(card),contentRect:{left:+left.toFixed(2),top:+top.toFixed(2),right:+right.toFixed(2),bottom:+bottom.toFixed(2),width:+(right-left).toFixed(2),height:+(bottom-top).toFixed(2)},usedAreaRatio:+(((right-left)*(bottom-top))/(r.width*r.height)).toFixed(4),emptyArea:+((r.width*r.height)-((right-left)*(bottom-top))).toFixed(2),clipped:children.filter(e=>e.scrollWidth>e.clientWidth+1||e.scrollHeight>e.clientHeight+1).map(e=>({className:e.className||e.tagName,client:[e.clientWidth,e.clientHeight],scroll:[e.scrollWidth,e.scrollHeight]})),overlaps,backgroundImage:getComputedStyle(card).backgroundImage};});
