...
const metrics=`(()=>{
 ...
 const all=[...document.querySelectorAll('.reviewOrderCard')],shown=all.filter(visible),cards=shown.map(card=>{const financial=card.querySelector('.reviewOrderTotal'),content=card.querySelector('.reviewMenuTable');return {index:Number(card.dataset.orderIndex),rect:rect(card),financial:financial?rect(financial):null,content:content?rect(content):null,text:card.textContent.trim()}});
...
async function exercise(win,{wheel=true,touch=true}={}){
 const before=await win.webContents.executeJavaScript(`(()=>{const e=document.querySelector('.reviewOrderList'),r=e.getBoundingClientRect();e.scrollTop=0;return {clientHeight:e.clientHeight,scrollHeight:e.scrollHeight,maxScrollTop:Math.max(0,e.scrollHeight-e.clientHeight),rect:{left:r.left,top:r.top,right:r.right,bottom:r.bottom},usableBottom:r.bottom}})()`,true);
...
runElectronVerification({app},async lifecycle=>{
 ...
 await resize(touchWin,834,1112);await touchWin.webContents.executeJavaScript(fixture(scenarios.find(x=>x[0]==='orders-10')[1]),true);await wait(touchWin);await touchWin.webContents.executeJavaScript(`document.querySelector('.reviewOrderList').scrollTop=240`,true);const dynamic=[];for(const height of [1112,940]){await resize(touchWin,834,height);dynamic.push({height,scrollTop:await touchWin.webContents.executeJavaScript(`document.querySelector('.reviewOrderList').scrollTop`,true),metrics:await touchWin.webContents.executeJavaScript(metrics,true)})}
