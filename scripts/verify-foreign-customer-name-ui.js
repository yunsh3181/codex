...
runElectronVerification({app},async lifecycle=>{
 ...
 for(const [viewport,width,height] of [['ipad-air3',834,1112],['windows',1280,720]]){
  ...
  for(const language of ['ko','en','ja','zh','vi','es'])results[viewport][language]=await win.webContents.executeJavaScript(`(()=>{setLanguage('${language}');Object.assign(state,{step:'phone',phone:'',customerDisplayName:''});render();if(foreignCustomerIdentity())for(const character of Array.from('ABCDEFGHIJK'))pressCustomerName(character);const input=document.querySelector('#customerNameDisplay'),body=document.body,root=document.documentElement;return {step:state.step,foreign:foreignCustomerIdentity(),input:input?{readOnly:input.readOnly,inputMode:input.inputMode,value:input.value,length:window.PJCustomerIdentity.graphemes(input.value).length,whiteSpace:getComputedStyle(input).whiteSpace}:null,keyCount:document.querySelectorAll('.customerNameKeyRow button').length,notice:document.querySelector('.notice')?.textContent||'',overflowX:Math.max(body.scrollWidth,root.scrollWidth)-innerWidth,overflowY:Math.max(body.scrollHeight,root.scrollHeight)-innerHeight}})()`,true)
 }
