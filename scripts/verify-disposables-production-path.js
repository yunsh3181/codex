...
runElectronVerification({app},async lifecycle=>{
 const isolation=installCustomerBrowserBootstrap({session:session.defaultSession,root,profile:userData,online:true});app.once('will-quit',()=>isolation.dispose());
 ...
 await win.loadFile(path.join(root,'index.html'));await wait(win);await isolation.verify(win);const needed=await runChoice(win,true);
 ...
 const payload=needed.saved.orders[0];await win.loadFile(isolation.document('admin/index.html'));await wait(win);
 ...
 const firestoreIsolation=await isolation.verify(win,{checkOnline:false});await lifecycle.writeReportAtomically(reportPath,{needed,notNeeded,admin,firestoreIsolation})
});
